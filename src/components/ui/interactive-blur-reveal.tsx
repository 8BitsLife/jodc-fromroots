...
//
// Integrated into JODC with two deliberate changes, both about weight and
// reliability — the API and the shader are untouched:
//   1. The default base image shipped as a ~250KB base64 JPEG that would live
//      in the bundle whether used or not. It now falls back to a procedural
//      brand texture (see @/lib/backdrop-textures).
//   2. The default noise texture was fetched from a third-party CDN. It is now
//      generated locally, so there is no extra request and no CORS surface.
// Pass `iChannel0` / `iChannel1` to supply your own images, exactly as before.
...
export interface InteractiveBlurRevealProps {
  ...
  duration?: number;
  /**
   * Multiplies the device pixel ratio the canvas renders at (0.5 - 1).
   * The effect is heavily blurred, so rendering below 1:1 is invisible and
   * meaningfully cheaper on weak GPUs. Added for this integration.
   */
...
function InteractiveBlurReveal({
  ...
  useEffect(() => {
    configRef.current = {
      ...
      resolutionScale: clampNumber(resolutionScale, 0.5, 1, 1),
    };
  }, [duration, mouseInteraction, mouseRadius, resolutionScale]);
    ...
    async function init() {
      ...
      const [baseImage, noiseImage] = await Promise.all([
        loadImage(
          iChannel0 ??
            createBrandBackdropDataUrl(
              Math.min(Math.round(window.innerWidth * 1.1), 1920),
              Math.min(Math.round(window.innerHeight * 1.1), 1200),
            ),
        ),
        loadImage(iChannel1 ?? createNoiseDataUrl()),
      ]);
      ...
      function resize() {
        const devicePixelRatio =
          ...
          configRef.current.resolutionScale;
