...
// Integration notes (JODC):
// - `LinePath` is exported so site sections can reuse the stroke on their own
//   copy. `stroke`, `strokeWidth` and `range` are optional and default to the
//   original values, so `Skiper19` renders exactly as shipped.
// - The unused default React import was dropped to satisfy `noUnusedLocals`.
...
const LinePath = ({
  ...
  /** Stroke colour. Defaults to the original lime. */
  stroke?: string;
  strokeWidth?: number;
  /** How much of the path is drawn at scroll progress 0 and 1. */
  range?: [number, number];
}) => {
  const pathLength = useTransform(scrollYProgress, [0, 1], range);
